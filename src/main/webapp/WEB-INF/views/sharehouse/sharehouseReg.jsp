...
<head>
  ...
  <style>
    ...
    .tag-select-btn:hover { transform: translateY(-2px); }
    .tag-select-btn i { margin-right: 8px; }
    ...
    .floor-input-btn:hover { transform: translateY(-2px); }
    .floor-input-btn i { margin-right: 8px; }
    ...
    .floor-modal {
      ...
      background-color: rgba(0,0,0,.5); z-index: 10001; align-items: center; justify-content: center;
    }
    .floor-modal-content {
      background: #fff; padding: 30px; border-radius: 12px; width: 90%; max-width: 400px;
      box-shadow: 0 4px 20px rgba(0,0,0,.2);
    }
    .floor-modal-title { font-size: 1.3rem; font-weight: 700; margin-bottom: 20px; color: #1c407d; }
    .floor-input-group { display: flex; align-items: center; gap: 10px; margin-bottom: 20px; }
    ...
    .floor-modal-btn {
      padding: 10px 20px; border: none; border-radius: 8px; font-size: .95rem; font-weight: 600; cursor: pointer; transition: .2s;
    }
    .floor-modal-btn.cancel { background: #f0f0f0; color: #666; }
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.sh-uploader-5 .uploader input[type="file"]').forEach(input => {
      ...
      wrap.addEventListener('mousedown', () => { input.value = null; });
...
<script>
  ...
  function closeFloorModal() { document.getElementById('floorModal').style.display = 'none'; }
  function saveFloorNumber() {
    const input = document.getElementById('floorInput');
    const floorNum = parseInt(input.value);
    if (isNaN(floorNum) || floorNum < 1 || floorNum > 99) { alert('1층부터 99층까지 입력 가능합니다.'); return; }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (floorInput) floorInput.addEventListener('keypress', (e) => { if (e.key === 'Enter') saveFloorNumber(); });
  });
</script>
...
<style id="tagModalStyles">
  ...
  html body #tagSelectModalOverlay .modal-title-text{ font-weight:700; font-size:1.05rem; color:#1f2937; }
  html body #tagSelectModalOverlay .modal-close{
    position:absolute; right:12px; top:12px; width:36px; height:36px; border:0; border-radius:999px;
  ...
  @media (max-width:640px){
    html body #tagSelectModalOverlay .all-tag-list{ grid-template-columns:repeat(2, minmax(0,1fr)); }
  }
  html body #tagSelectModalOverlay .tag-btn{
    padding:10px 12px; border:1px solid #e5e8ef; background:#fff; color:#334155;
    border-radius:999px; font-size:.95rem; cursor:pointer; transition:.18s;
